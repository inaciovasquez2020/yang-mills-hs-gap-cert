# Repository Status (Authoritative)

Status = Negative Result Certified + Conditional Reduction Framework + Open Core Lemma

## Established
- Certified obstruction to Hilbert–Schmidt coercivity route
- Verified operator identities, scaling laws, and counterexamples
- Reproducible CI-backed experimental and analytic artifacts

## Conditional
- Reduction of Yang–Mills mass gap to block spectral gap
- Operator coercivity → Poincaré → exponential decay chain

## Open (Core Lemma)
- Uniform bound:
  sup_U ||∇²S_B(U) - βΔ_B|| ≤ C L^-2
- Equivalent: block spectral gap γ_B ≥ c L^-2

## Interpretation
This repository does not solve the Yang–Mills mass gap unconditionally.
It certifies obstructions, conditional reductions, and executable verification artifacts.

Canonical target file: open_problems/BLOCK_SPECTRAL_GAP_CORE_LEMMA.md
