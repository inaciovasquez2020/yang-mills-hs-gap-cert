...
def test_confining_kernel_contracts():
    for m in [8, 16, 32]:
        for g in [0.2, 0.5, 1.0]:
            ...
            # Confining kernels should have contraction rate < 1
            ...
            assert rate < 1.0
            # For g large enough, should contract well
            if g >= 1.0 and m <= 16:
                assert rate < 0.99
...
def test_uniform_kernel_no_gap():
    m = 8
    W_unif = np.ones((m, m))
    # Uniform kernel should have spectral gap = 0 (second eigenvalue = 0)
    # But Dobrushin coefficient = 1 (no contraction)
    rate = transfer_matrix_contraction_rate(W_unif)
    assert abs(rate - 1.0) < 1e-10
    # Weyl obstruction should detect no gap
