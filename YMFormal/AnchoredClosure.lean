-- ── Stub declarations (opaque axioms) for all identifiers assumed by the
--    theorem bodies.  Replace each one with a real definition to discharge
--    the corresponding obligation.
variable {α : Type*}

opaque Plaquette : Type
opaque AnchoredPatch : Type
opaque AnchoredCover : Type

opaque carrier      : AnchoredPatch → Finset Plaquette
opaque merge        : AnchoredPatch → AnchoredPatch → AnchoredPatch
opaque valuation    : AnchoredPatch → ℝ
opaque hessian      : AnchoredPatch → AnchoredPatch  -- placeholder type
opaque interiorHessian : AnchoredPatch → AnchoredPatch
opaque boundaryHessian : AnchoredPatch → AnchoredPatch
opaque lambdaMin    : AnchoredPatch → ℝ
opaque energy       : AnchoredCover → ℝ
opaque step         : AnchoredCover → AnchoredCover
opaque stepN        : AnchoredCover → ℕ → AnchoredCover
opaque StableOnAnchor : AnchoredPatch → Prop
opaque contractionRate : AnchoredCover → ℝ
opaque patchEnergy  : AnchoredPatch → AnchoredCover → ℝ
opaque stableDecay  : AnchoredPatch → ℝ

-- Auxiliary lemmas assumed from geometry / Mathlib
opaque merge_carrier_eq (P Q : AnchoredPatch) (h : Disjoint (carrier P) (carrier Q)) :
    carrier (merge P Q) = carrier P ∪ carrier Q
opaque boundaryHessian_posSemidef (P : AnchoredPatch) : (0 : ℝ) ≤ 0  -- placeholder
opaque rayleigh_ge_lambdaMin : True
opaque lambdaMin_add_psd_mono : True
opaque stableOnAnchor_of_spectral_floor : True
opaque contractionRate_lt_one : True
opaque contractionRate_bound : True
opaque sum_step_energy_bound : True
opaque mainEnergy_empty : True
opaque mainEnergy_insert : True
